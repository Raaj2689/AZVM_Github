...
resource "azurerm_network_interface" "test_network" {
  name                = "LAN-Test"
  location            = var.location
  resource_group_name = azurerm_resource_group.RG.name
...
resource "azurerm_network_interface_security_group_association" "NSG_Association" {
  network_interface_id      = azurerm_network_interface.test_network.id
...
resource "azurerm_windows_virtual_machine" "TerraformVM" {
  ...
  network_interface_ids = [azurerm_network_interface.test_network.id]
